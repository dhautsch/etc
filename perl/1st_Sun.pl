...
my @FLG = qw(today list test last next);
my $WDAY;
my $PAT_WEEK = join('|', @WEEK);
my $PAT_DAY = join('|', @WDAY);
my $PAT = '^(' . "($PAT_DAY)" . "|(" . "($PAT_WEEK)_" . "($PAT_DAY)))\$";
my $WDAY_CNT;
my %OPTS;
my @OUT;
my $EXIT = 1;

if (GetOptions(\%OPTS, 'help', @FLG) && (@ARGV == 1 && ! ($OPTS{today} || $OPTS{list}) && $ARGV[0] =~ m!$PAT!) || (@ARGV == 0 && ($OPTS{today} || $OPTS{list}))) {
    my $day_ = $2 ? $2 : ($5 ? $5 : $WDAY[$TIME[6]]);
    my $wk_ = $4 ? $4 : 0;
...
if ($OPTS{help}) {
    print "Usage:\t$0 -today\n";
    print "Usage:\t$0 -list\n";
    print "\t$0 [-test|-last|-next] $PAT\n";
    print "\t-today: print date, gmt noon and day of week\n";
    print "\t-list: print date, gmt noon and day of week +- one year\n";
...
foreach my $wday_ ($OPTS{list} ? (0..$#WDAY): ($WDAY)) {
    foreach my $year_ (sort keys %H) {
        foreach my $month_ (sort keys %{$H{$year_}}) {
            my @a_ = @{$H{$year_}{$month_}{WDAY}[$wday_] ? $H{$year_}{$month_}{WDAY}[$wday_] : []};
            ...
            foreach (@a_) {
                if ($OPTS{test}) {
                    ...
                    push @OUT, "$_->{DATE} $_->{NOON} $WEEK[$i_]_$WDAY[$wday_]\n";
                }
